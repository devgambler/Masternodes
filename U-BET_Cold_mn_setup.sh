...
echo -e "${NC}Your U-BET Masternode (MN) Config file is:" 
echo -e "${GREEN}MN1 $myip:$DEFAULTUBETPORT $PRIV_KEY${NC}"
echo -e "${NC}You must add Masternode_Private_Key Transaction_ID Transaction_Index"
echo -e "${NC}Right after you need to save the 'masternode.conf' file. Then you need to wait for at least 15 confirmations for the 2,500 U-BET coins collateral transaction and then restart the wallet (completely close and then run your wallet again). Then go to the 'Masternode' tab in the wallet and start your U-BET masternode.${NC} "
